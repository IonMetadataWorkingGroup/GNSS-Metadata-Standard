...
namespace GnssMetadata
{
	...
	public:
		...
		Block(const IonString& id = "", bool bIsReference = false)
				: AttributedObject(id, bIsReference), _cycles(0), _sizeHeader(0), _sizeFooter(0)
		{}
		...
		Block(const Block& rhs)
				: AttributedObject(rhs), _cycles(rhs._cycles), _sizeHeader(rhs._sizeHeader), _sizeFooter(rhs._sizeFooter), _chunklist(rhs._chunklist)
		{}
		 ...
		 */
		bool IsDefined() const { return _cycles > 0 && _chunklist.size() > 0; }
		 ...
		 */
		void Cycles(const size_t cycles) { _cycles = cycles; }

		/**
		 * Gets the size of the header in bytes..
		 */
		size_t SizeHeader() const { return _sizeHeader; }

		/**
		 * Sets the size of the footer in bytes.
		 */
		void SizeHeader(const size_t size) { _sizeHeader = size; }

		/**
		 * Gets the size of the footer in bytes..
		 */
		size_t SizeFooter() const { return _sizeFooter; }

		/**
		 * Sets the size of the footer in bytes.
		 */
		void SizeFooter(const size_t size) { _sizeFooter = size; }

		const GnssMetadata::ChunkList& Chunks() const { return _chunklist; }
		GnssMetadata::ChunkList& Chunks() { return _chunklist; }
