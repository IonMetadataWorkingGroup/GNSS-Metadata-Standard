...
namespace GnssMetadata
{
	...
	public:
		...
		System(const IonString& id = "", bool bIsReference = false) : AttributedObject(id, bIsReference), _type(Undefined) {}

		System(SystemType type, const IonString& id = "", bool bIsReference = false) : AttributedObject(id, bIsReference), _type(type) {}
		...
		const Frequency& BaseFrequency() const { return _freqBase; }
		Frequency& BaseFrequency() { return _freqBase; }
		void BaseFrequency(const Frequency& centerFrequency) { _freqBase = centerFrequency; }

		/**
		 * Gets the system equipment information.
		 */
		IonString Equipment() const { return _equipment; }
		/**
		 * Sets the system equipment information.
		 */
		void Equipment(const IonString& equipment) { _equipment = equipment; }
		 ...
		 */
		void Type(const SystemType type) { _type = type; }

		const GnssMetadata::SourceList& Sources() const { return _sourcelist; }
		GnssMetadata::SourceList& Sources() { return _sourcelist; }
		void AddSource(const GnssMetadata::Source& source) { _sourcelist.push_back(source); }

		const GnssMetadata::ClusterList& Clusters() const { return _clusterlist; }
		GnssMetadata::ClusterList& Clusters() { return _clusterlist; }
		void AddCluster(const GnssMetadata::Cluster& cluster) { _clusterlist.push_back(cluster); }
