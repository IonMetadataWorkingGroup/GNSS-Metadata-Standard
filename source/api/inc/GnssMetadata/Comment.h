...
namespace GnssMetadata
{
	...
	public:
		 ...
		 */
		Comment(const IonString& value, CommentFormat format) : _format(format), _value(value) {}

		Comment(const Comment& rhs) : _format(rhs._format), _value(rhs._value) {}

		/**
		 * Gets the format of the comment.
		 */
		CommentFormat Format() const { return _format; }

		/**
		 * Sets the comment value and format.
		 */
		void Value(const IonString& value, const CommentFormat /*format*/) { _value = value; }

		/**
		 * Gets the comments value.
		 */
		IonString Value() const { return _value; }
