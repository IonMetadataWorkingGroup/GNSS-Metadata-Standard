...
namespace GnssMetadata
{
	 ...
	 * point for creating metadata for data files.
	 */
	...
	public:
		...
		const AnyUriList& Includes() const { return _includes; }
		AnyUriList& Includes() { return _includes; }

		const FileList& Files() const { return _files; }
		FileList& Files() { return _files; }

		const FileSetList& FileSets() const { return _filesets; }
		FileSetList& FileSets() { return _filesets; }

		const BandList& Bands() const { return _bands; }
		BandList& Bands() { return _bands; }

		const StreamList& Streams() const { return _streams; }
		StreamList& Streams() { return _streams; }

		const LumpList& Lumps() const { return _lumps; }
		LumpList& Lumps() { return _lumps; }

		const ChunkList& Chunks() const { return _chunks; }
		ChunkList& Chunks() { return _chunks; }

		const BlockList& Blocks() const { return _blocks; }
		BlockList& Blocks() { return _blocks; }

		const LaneList& Lanes() const { return _lanes; }
		LaneList& Lanes() { return _lanes; }

		const SessionList& Sessions() const { return _sessions; }
		SessionList& Sessions() { return _sessions; }

		const SystemList& Systems() const { return _systems; }
		SystemList& Systems() { return _systems; }
