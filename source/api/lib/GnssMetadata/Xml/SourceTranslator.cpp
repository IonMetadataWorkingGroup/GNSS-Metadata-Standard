...
Source::SourceType ToSourceType(const char* psz)
{
	...
	{
		if (strcmp(_szSourceType[i], psz) == 0) return (Source::SourceType)i;
	}
...
Source::SourcePolarization ToSourcePolarization(const char* psz)
{
	...
	{
		if (strcmp(_szSourcePolarization[i], psz) == 0) return (Source::SourcePolarization)i;
	}
...
bool SourceTranslator::OnRead(Context & ctxt, const XMLElement & elem, AccessorAdaptorBase* pAdaptor)
{
	...
	if (!ReadAttributedObject(source, ctxt, elem)) return false;
	...
	if (bRetVal) pAdaptor->set(&source);
	return bRetVal;
}
