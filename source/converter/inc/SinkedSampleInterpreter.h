...
{
protected:
	uint32_t mChunkIndex;        // which element of the chunk array to access
	uint32_t mResidualBitIndex;  // how many bits into the mChunkIndex^th element to the start of the sample
	SampleSink* mSampleSink;     // the associated sample sink, where the interpreted samples will be put
...
public:
	...
	SinkedSampleInterpreter(
	        ...
	        void (*formatFunc)(sample_base_t (*)(const chunk_t*, uint32_t, uint32_t, uint32_t), const chunk_t* pChunk, uint32_t, uint32_t, uint32_t,
	                SampleSink*),
