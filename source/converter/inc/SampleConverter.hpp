...
template<typename sample_base_t>
bool SampleConverter::CreateBlockInterpreter(GnssMetadata::Metadata& md, SampleStreamInfo commonSampleInfo, GnssMetadata::Block* block,
        BlockInterpreter** blockInterp)
{
	...
	for (GnssMetadata::LumpList::iterator lpIt = chunk->Lumps().begin(); lpIt != chunk->Lumps().end(); ++lpIt)
	{
		...
		for (uint16_t lr = 0; lr < lnLumpRepeat; lr++)
		{
			...
			for (GnssMetadata::StreamList::iterator smIt = lpIt->Streams().begin(); smIt != lpIt->Streams().end(); ++smIt)
			{
				...
				uint16_t numPaddingBits = static_cast<uint32_t>(smIt->Packedbits()
				        - numSmpInterp * (chunkIntrp->mSampleInterpFactory.BitWidth(smIt->Format(), smIt->Quantization())));
				...
				*sampleInfo = commonSampleInfo;
				...
				sampleInfo->mIsComplexSamples = (smIt->Format() != GnssMetadata::IonStream::SampleFormat::IF
				        && smIt->Format() != GnssMetadata::IonStream::SampleFormat::IFn);
