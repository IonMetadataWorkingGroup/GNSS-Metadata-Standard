...
public:
	...
	virtual uint32_t GetSamples(const int8_t*& pbuff) const
	{
		return DoGetSamples(reinterpret_cast<const void**>(&pbuff));
	}

	virtual uint32_t GetSamples(const int16_t*& pbuff) const
	{
		return DoGetSamples(reinterpret_cast<const void**>(&pbuff));
	}

	virtual uint32_t GetSamples(const int32_t*& pbuff) const
	{
		return DoGetSamples(reinterpret_cast<const void**>(&pbuff));
	}

	virtual uint32_t GetSamples(const int64_t*& pbuff) const
	{
		return DoGetSamples(reinterpret_cast<const void**>(&pbuff));
	}

	virtual uint32_t GetSamples(const float*& pbuff) const
	{
		return DoGetSamples(reinterpret_cast<const void**>(&pbuff));
	}

	virtual uint32_t GetSamples(const double*& pbuff) const
	{
		return DoGetSamples(reinterpret_cast<const void**>(&pbuff));
	}
